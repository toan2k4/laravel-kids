        ...
        <div class="row">
                ...
                <div class="mb-3">
                    ...
                    <select name="supplier_id" id="supplier_id" class="form-control">
                        ...
                        @foreach ($suppliers as $key => $item)
                            <option value="{{ $key }}" @selected($product->supplier_id == $key)>{{ $item }}</option>
                        @endforeach
                    </select>
                    @error('supplier_id')
                        <div class="text-danger">{{ $message }}</div>
            ...
            <div class="col-6">
                <div class="mb-3">
                    ...
                    <textarea name="description" class="form-control" id="description" cols="10" rows="4">{{ $product->description }}</textarea>
                    @error('description')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="file" class="form-control" id="product_image" name="image"
                        value="{{ $product->image }}">
                        <img src="{{ \Storage::url( $product->image)}}" width="50px">
                    @error('image')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
